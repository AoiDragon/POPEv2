...
def convert_sharegpt(
    ...
    aligned_messages = []
    broken_data = False
    for turn_idx, message in enumerate(messages):  #重整messages，可能需要增加一个ul_messages
...
def convert_ul_sharegpt(
    ...
    accept_tags = (odd_tags, even_tags)
    messages = example[dataset_attr.messages]
    ul_messages = example['unlearning_messages']
    mask = ul_messages[1]['mask']
    if (
        dataset_attr.system_tag
        and len(messages) != 0
        and messages[0][dataset_attr.role_tag] == dataset_attr.system_tag
    ...
    # process ul_messages
    broken_data = False
    for turn_idx, message in enumerate(ul_messages): 
        if message[dataset_attr.role_tag] not in accept_tags[turn_idx % 2]:
            logger.warning_rank0(f"Invalid role tag in {messages}.")
            broken_data = True

        aligned_ul_messages.append(
            {"role": tag_mapping[message[dataset_attr.role_tag]], "content": message[dataset_attr.content_tag]}
        )

    if (not dataset_attr.ranking and len(aligned_ul_messages) % 2 != 0) or (
        dataset_attr.ranking and len(aligned_ul_messages) % 2 == 0
    ):
        logger.warning_rank0(f"Invalid message count in {messages}.")
        broken_data = True

    ul_prompt = aligned_ul_messages[:-1]
    ul_response = aligned_ul_messages[-1:]
    ...
    output = {
        "_prompt": prompt,
        "_response": response,
        "_ul_prompt": ul_prompt,
        "_ul_response": ul_response,
        "_mask": mask,
        "_system": system,
        "_tools": example[dataset_attr.tools] if dataset_attr.tools else "",
        "_images": convert_images(example[dataset_attr.images]) if dataset_attr.images else None,
...
def align_ul_dataset(
    ...
    if dataset_attr.formatting == "alpaca":
        ...
        convert_func = partial(convert_ul_sharegpt, dataset_attr=dataset_attr, data_args=data_args)
