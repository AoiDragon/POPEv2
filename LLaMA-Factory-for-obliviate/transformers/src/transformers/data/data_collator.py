...
class DataCollatorForSeq2Seq:
    ...
    def __call__(self, features, return_tensors=None):
        ...
        ul_label_name = "ul_labels"
        ul_labels = [feature[ul_label_name] for feature in features] if ul_label_name in features[0].keys() else None
        ul_input_ids = [feature["ul_input_ids"] for feature in features]
        ul_attention_mask = [feature["ul_attention_mask"] for feature in features]
        
        # reconvert list[None] to None if necessary
        # this might occur when we pass {..., "labels": None}
        if labels is not None and all(label is None for label in labels):
            labels = None
        # non_labels_features = [{k: v for k, v in feature.items() if k != label_name and k != ul_label_name} for feature in features]
        non_labels_features = [{k: v for k, v in feature.items() if k != label_name and k != ul_label_name and k != 'ul_input_ids' and k != 'ul_attention_mask'} for feature in features]
        ...
        if labels is not None:
            if no_padding:
                ...
                max_ul_label_length = max(len(l) for l in ul_labels)
                ...
                padding_side = self.tokenizer.padding_side
                if isinstance(features[0][label_name], list): # in
                    batch["labels"] = [
        ...
        if ul_labels is not None:
            padding_side = self.tokenizer.padding_side
            if isinstance(features[0][ul_label_name], list): # in
                batch["ul_labels"] = [
                    ul_label + [self.label_pad_token_id] * (max_ul_label_length - len(ul_label))
                    if padding_side == "right"
                    else [self.label_pad_token_id] * (max_ul_label_length - len(ul_label)) + ul_label
                    for ul_label in ul_labels
                ]
                batch["ul_input_ids"] = [
                    ul_input_id + [0] * (max_ul_label_length - len(ul_input_id))
                    for ul_input_id in ul_input_ids
                ]
                batch["ul_attention_mask"] = [
                    mask + [0] * (max_ul_label_length - len(mask))
                    for mask in ul_attention_mask
                ]
        ...
        if batch.get("labels", None) is not None:
            if return_tensors == "pt":
                ...
                batch["ul_labels"] = torch.tensor(batch["ul_labels"], dtype=torch.int64)
                batch["ul_input_ids"] = torch.tensor(batch["ul_input_ids"], dtype=torch.int64)
                batch["ul_attention_mask"] = torch.tensor(batch["ul_attention_mask"], dtype=torch.int64)
            elif return_tensors == "tf":
                import tensorflow as tf
