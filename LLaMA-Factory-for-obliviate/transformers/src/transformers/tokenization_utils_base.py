...
class PreTrainedTokenizerBase(SpecialTokensMixin, PushToHubMixin):
    ...
    def _pad(
        ...
        required_input = encoded_inputs[self.model_input_names[0]]
        # ul_input_ids = encoded_inputs["ul_input_ids"]
        # max length需要设定为ul_input_ids和input_ids的最大长度
        # if padding_strategy == PaddingStrategy.LONGEST:
        #     max_length = max(len(required_input), ul_input_ids)

        if max_length is not None and pad_to_multiple_of is not None and (max_length % pad_to_multiple_of != 0):
            max_length = ((max_length // pad_to_multiple_of) + 1) * pad_to_multiple_of

        # needs_to_be_padded = padding_strategy != PaddingStrategy.DO_NOT_PAD and (len(required_input) != max_length or len(ul_input_ids) != max_length)
        ...
        if needs_to_be_padded:
            ...
            # ul_difference = max_length - len(encoded_inputs["ul_input_ids"])
            padding_side = padding_side if padding_side is not None else self.padding_side

            if padding_side == "right":
                # encoded_inputs["ul_input_ids"] = encoded_inputs["ul_input_ids"] + [0] * ul_difference
                # encoded_inputs["ul_attention_mask"] = encoded_inputs["ul_attention_mask"] + [0] * ul_difference
