...
class Qwen2VLForConditionalGeneration(Qwen2VLPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        # ul process
        ul_input_embeds = self.model.embed_tokens(ul_input_ids)
        ul_image_mask = (
                    (ul_input_ids == self.config.image_token_id)
                    .unsqueeze(-1)
                    .expand_as(ul_input_embeds)
                    .to(ul_input_embeds.device)
                )
        ul_inputs_embeds = ul_input_embeds.masked_scatter(ul_image_mask, image_embeds)
        ul_attention_mask = ul_attention_mask.to(ul_inputs_embeds.device)
        ...
        # ul
        ul_outputs = self.model(
            input_ids=None,
            position_ids=ul_position_ids,
        ...
        hidden_states = ul_outputs[0]
        logits = self.lm_head(hidden_states)
        
        logits = logits.float()
        # Shift so that tokens < n predict n
        shift_logits = logits[..., :-1, :].contiguous()
        shift_labels = ul_labels[..., 1:].contiguous()
        loss_fct = CrossEntropyLoss()
        shift_logits = shift_logits.view(-1, self.config.vocab_size)
        shift_labels = shift_labels.view(-1)
        # Enable model parallelism
        shift_labels = shift_labels.to(shift_logits.device)
        ul_loss = loss_fct(shift_logits, shift_labels)
