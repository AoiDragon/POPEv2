...
class LlavaNextForConditionalGeneration(LlavaNextPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        ul_inputs_embeds = self.get_input_embeddings()(ul_input_ids)
        ...
        if pixel_values is not None and pixel_values.size(0) > 0:
            ...
            ul_special_image_mask = (ul_input_ids == self.config.image_token_index).unsqueeze(-1)
            ul_special_image_mask = ul_special_image_mask.expand_as(ul_inputs_embeds).to(ul_inputs_embeds.device)
            ul_inputs_embeds = ul_inputs_embeds.masked_scatter(ul_special_image_mask, image_features)
        ...
        ul_outputs = self.language_model(
            attention_mask=ul_attention_mask,
            position_ids=position_ids,
            past_key_values=past_key_values,
            inputs_embeds=ul_inputs_embeds,
            use_cache=use_cache,
            output_attentions=output_attentions,
            output_hidden_states=output_hidden_states,
            return_dict=return_dict,
        ...
        ul_logits = ul_outputs[0]
        shift_ul_attention_mask = ul_attention_mask[:, -(ul_logits.shape[1] - 1) :].to(ul_logits.device)
        shift_ul_logits = ul_logits[..., :-1, :][shift_ul_attention_mask.to(ul_logits.device) != 0].contiguous()
        shift_ul_labels = ul_labels[..., 1:][shift_ul_attention_mask.to(ul_labels.device) != 0].contiguous()
        ul_loss = loss_fct(
                shift_ul_logits.view(-1, shift_ul_logits.size(-1)), shift_ul_labels.view(-1).to(shift_ul_logits.device)
            )
        loss = loss - alpha * ul_loss
